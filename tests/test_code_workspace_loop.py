...
class CodeWorkspaceStaticContractTest(unittest.TestCase):
    ...
    def test_sdk_result_errors_and_reconnect_controls_are_not_silently_completed(self):
        ...
        self.assertNotIn('id="cwComposerReconnectBtn"', source)
        self.assertNotIn("trigger: 'reconnect'", source)
        self.assertIn("obj.status === 'reconnect_failed'", source)
        self.assertIn("await persistCodeQueueEntry(entry)", source)
